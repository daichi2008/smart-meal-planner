...
function SavedInner() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      {error && (
        <p className="mb-4 rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-800">{error}</p>
      )}
      ...
      {fetching ? (
        ...
        <Card className="border border-dashed border-gray-300 p-12 text-center">
          <p className="text-lg text-gray-600">🍽️ {t('savedEmpty')}</p>
          <p className="mt-2 text-sm text-gray-400">{t('savedEmptyHint')}</p>
          <Button
            onClick={() => router.push('/dashboard')}
